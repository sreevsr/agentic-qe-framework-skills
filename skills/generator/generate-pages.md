...
## PROHIBITED ACTIONS — Helper Boundary (violation = pipeline failure)

Page objects generated by this skill contain ONLY methods that map to locator JSON elements: navigate, click, fill, getText, getters for CAPTURE targets. They MUST NOT contain custom business logic.

**NEVER add methods to a base page object that:**
- Match any `USE_HELPER: PageName.methodName` reference in the scenario
- Perform multi-element calculations (e.g., summing prices, counting totals)
- Perform bulk operations (e.g., removing all items, clearing forms)
- Implement business validation logic (e.g., price verification, data comparison)

**These methods belong EXCLUSIVELY in `*.helpers.ts` files**, which are team-owned and created by the team — not by the pipeline.

**If a scenario uses USE_HELPER and no helpers file exists:**
- The method DOES NOT EXIST. Do not create it.
- The spec will contain a warning comment (see `generate-web-spec` skill).
- The test will fail or be marked `test.fixme()` by the Healer. This is correct.

**Why:** Base page objects (`{PageName}.ts`) are pipeline-owned. They are regenerated on every pipeline run. Any custom method added here will be overwritten. Team-maintained logic survives only in `*.helpers.ts` files.
...
- [ ] No method in a base page object matches any USE_HELPER reference in the scenario
